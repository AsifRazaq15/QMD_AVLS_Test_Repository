...
internal class Program
{
	...
	private static void Main(string[] args)
	{
		...
		try
		{
			...
			{
				array = JArray.Parse(lastObservationData.Item1);
			}
			else
			{
				string text = ExtractBody(lastObservationData.Item1);
				logger.Error("Error Message: " + text);
				Environment.Exit(0);
			}
			string text2 = Guid.NewGuid().ToString();
			logger.Info("Batch ID: " + text2);
			logger.Info("RequestTime: " + _requestTime);
			logger.Info("ResponseTime: " + _responseTime);
			logger.Info("Response Message: " + lastObservationData.Item1.Substring(0, 100) + "...");
			using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["ODW_STG_ConnectionString"].ConnectionString))
			{
				...
				using (SqlCommand sqlCommand = new SqlCommand())
				{
					...
					sqlCommand.Parameters.Add("@ResponsePayload", SqlDbType.NVarChar, -1).Value = lastObservationData.Item1;
					...
					sqlCommand.Parameters.Add("@Batch_Id", SqlDbType.NVarChar, 256).Value = text2;
	...
	{
		int num = html.IndexOf("<body>") + "<body>".Length;
		int length = html.IndexOf("<br>") - num;
		return html.Substring(num, length);
	}

	public static Tuple<string, string> GetLastObservationData()
	{
		...
		string item = new StreamReader(httpWebResponse.GetResponseStream(), encoding).ReadToEnd();
		httpWebResponse.Close();
		_responseTime = DateTime.Now;
		return Tuple.Create(item, httpWebResponse.StatusCode.ToString());
	}
}
